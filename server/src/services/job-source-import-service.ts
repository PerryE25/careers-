...
import type { AtsProvider, JobTarget, Profile } from "../domain/models.js";
...
interface ParsedJobTargetCandidate {
  company?: string;
  roleTitle?: string;
  location?: string;
  applyUrl?: string;
  providerHint?: AtsProvider;
}

interface JobSourceParserInput {
  fileName: string;
  mimeType?: string;
  content: string;
}
...
interface JobSourceParser {
  canParse(input: JobSourceParserInput): boolean;
  parse(input: JobSourceParserInput): ParsedJobTargetCandidate[];
}

export interface ImportedJobTargetSummary {
  id: string;
  company?: string;
  title?: string;
  location?: string;
...
class JsonJobSourceParser implements JobSourceParser {
  canParse(input: JobSourceParserInput) {
    const extension = path.extname(input.fileName).toLowerCase();
    return extension === ".json" || input.mimeType?.includes("json") === true;
  }

  parse(input: JobSourceParserInput) {
...
class MarkdownJobSourceParser implements JobSourceParser {
  canParse(input: JobSourceParserInput) {
    const extension = path.extname(input.fileName).toLowerCase();
  ...
  parse(input: JobSourceParserInput) {
    ...
    const rows: ParsedJobTargetCandidate[] = [];
...
function extractJsonRows(parsed: unknown) {
...
function normalizeJsonCandidate(input: unknown): ParsedJobTargetCandidate {
  ...
  const providerHint = normalizeProviderHint(
    pickStringField(row, ["providerHint", "provider", "atsProvider"]),
  );
...
function normalizeProviderHint(value?: string): AtsProvider | undefined {
  if (!value) {
    return undefined;
  }
...
function pickStringField(row: Record<string, unknown>, fieldNames: string[]) {
...
function pickCell(row: Record<string, string>, candidates: string[]) {
...
function isRelevantRole(candidate: ParsedJobTargetCandidate) {
  const title = `${candidate.roleTitle ?? ""} ${candidate.company ?? ""}`.toLowerCase();
  const roleMatch =
    /(software|swe|backend|front ?end|full ?stack|platform|application|developer|engineer)/i.test(title);
  const seniorityMatch =
    /(new grad|new graduate|entry level|entry-level|graduate|university grad|early career|associate|junior)/i.test(title);
  return roleMatch || (roleMatch && seniorityMatch);
}

function locationPreferenceScore(location: string | undefined, profile: Profile | null) {
  if (!location) {
    return 0;
  }
  ...
  if (
    profile?.canonicalProfile.locationPreferences.remotePreference?.toLowerCase().includes("remote") &&
...
export class JobSourceImportService {
  private readonly parsers: JobSourceParser[] = [new JsonJobSourceParser(), new MarkdownJobSourceParser()];
  ...
  private findExplicitLocalSource() {
    const candidates = [
  ...
  private findLatestStoredSource() {
  ...
  private findLatestUploadSource() {
  ...
  private upsertParsedRows(parsedRows: ParsedJobTargetCandidate[], options: { relevantOnly?: boolean } = {}) {
    const seenUrls = new Set<string>();
    const seenJobs = new Set<string>();
    const imported: JobTarget[] = [];
  ...
  private summarizeJobs(jobs: JobTarget[]) {
  ...
  importLatestJobSource(options: { relevantOnly?: boolean } = {}) {
    const source = this.findLatestJobSource();
    if (!source) {
      throw new Error(
        "No saved job target source was found. Add `public/autofill/job-targets.json` or upload a markdown job list.",
      );
    }
  ...
  importLatestJobMarkdown(options: { relevantOnly?: boolean } = {}) {
    return this.importLatestJobSource(options);
  }

  listJobTargets(options: { relevantOnly?: boolean } = {}) {
    const source = this.findLatestJobSource();
    if (source) {
      const parsedRows = this.parseJobSource(source);
    ...
    return this.summarizeJobs(
      this.store
        .listJobs()
        .filter((job) => Boolean(job.sourceUrl))
        .filter((job) =>
          options.relevantOnly === false
            ? true
            : isRelevantRole({
                company: job.company,
                roleTitle: job.title,
...
function hasSupportedJobSourceExtension(fileName: string) {
