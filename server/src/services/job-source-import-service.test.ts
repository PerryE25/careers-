...
test("prefers public job-targets.json when it is available", () => {
  ...
  fs.writeFileSync(
    path.join(uploadsDir, "older-targets.md"),
    [
      "| Company | Role | Location | Apply |",
      "| --- | --- | --- | --- |",
  ...
  fs.writeFileSync(
    path.join(publicAutofillDir, "job-targets.json"),
    JSON.stringify([
      {
        company: "Figma",
  ...
  try {
    ...
    assert.deepEqual(
      result.targets.map((target) => target.company),
      ["Figma", "Ramp"],
    );
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
...
test("listJobTargets reflects the current explicit source instead of stale stored jobs", () => {
  ...
  createProfile(store);
  store.upsertJob(
    { normalizedUrl: "https://boards.greenhouse.io/figma/jobs/1234567" },
    {
      provider: "greenhouse",
  ...
  try {
    ...
    assert.deepEqual(
      targets.map((target) => target.company),
      ["Saronic", "Bolt"],
    );
    assert.deepEqual(
