...
import {
  ...
  WORKDAY_SELECTORS,
  WORKDAY_STEP_NAME_SELECTORS,
  WORKDAY_SUBMIT_BUTTON_SELECTORS,
} from "./configs/workday-selectors.js";
...
import { capturePageState, type PageStateSnapshot, waitForAnyVisible, waitForAsyncValidation, waitForPageReady, waitForPageStateChange } from "../shared/playwright-utils.js";
...
export class WorkdayAdapter extends BaseAtsAdapter {
  ...
  protected selectors = WORKDAY_SELECTORS;
  ...
  async fillField(page: Page, field: DetectedFormField, value: string, context: AutomationContext) {
    if (
      field.type === "checkbox" &&
  ...
  async answerScreeningQuestions(page: Page, context: AutomationContext) {
    ...
    context.logger?.info("Workday screening scan.", snapshot as unknown as Record<string, unknown>);
  }
  ...
  async reviewBeforeSubmit(page: Page, context: AutomationContext) {
    ...
    context.logger?.info("Workday review checkpoint reached.", snapshot as unknown as Record<string, unknown>);
  ...
  async collectErrors(page: Page, context: AutomationContext): Promise<AdapterCollectedError[]> {
  ...
  async fillApplication(page: Page, context: AutomationContext): Promise<AdapterRunResult> {
    const statusUpdates: AdapterRunResult["statusUpdates"] = [];
    const pushStatus = async (stage: string, detail: string, step?: string) => {
      const pageState: PageStateSnapshot = await capturePageState(page);
      statusUpdates.push({
        stage,
        detail,
        step,
    ...
    const allErrors: AdapterCollectedError[] = [];
    ...
    for (let iteration = 0; iteration < 8; iteration += 1) {
      ...
      if (
        snapshotBefore.stepName === snapshotNext.stepName &&
    ...
    const confidencePenalty =
      Math.min(uniqueErrors.length * 0.08, 0.24) + Math.min(unresolvedRequired.size * 0.07, 0.28);
    ...
    const reviewSummary = buildReviewSummary({
      ...
      validationErrors: uniqueErrors.map((error) =>
...
function sanitizeStepName(step: string) {
  return step.replace(/[^a-z0-9-_]+/gi, "-").toLowerCase();
}

function dedupeErrors(errors: AdapterCollectedError[]) {
