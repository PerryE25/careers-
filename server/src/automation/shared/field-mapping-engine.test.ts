...
test("maps consent-style checkboxes safely", () => {
  const decision = mapFieldToProfile(
    profile,
    field({
...
test("confidence summary does not collapse when only optional extras remain unresolved", () => {
  ...
  const unresolvedOptional = mapFieldToProfile(
    profile,
    field({
  ...
  const optionalDemographic = mapFieldToProfile(
    profile,
    field({
...
test("maps current company from autofill", () => {
  const extended: Profile = {
    ...profile,
    autofillFields: { ...profile.autofillFields, employer: "Contoso Labs" },
  };
  const decision = mapFieldToProfile(
    extended,
    field({ label: "Current company", type: "text", required: true, domLocator: { name: "org" } }),
  );
  assert.equal(decision.status, "resolved");
  assert.equal(decision.answer, "Contoso Labs");
});

test("maps how did you hear from autofill referral", () => {
  const extended: Profile = {
    ...profile,
    autofillFields: { ...profile.autofillFields, referral: "LinkedIn" },
  };
  const decision = mapFieldToProfile(
    extended,
    field({ label: "How did you hear about us?", type: "text", required: false }),
  );
  assert.equal(decision.status, "resolved");
  assert.equal(decision.answer, "LinkedIn");
});
