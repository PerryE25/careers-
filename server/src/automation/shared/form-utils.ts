...
export async function findFirstVisibleByLabels(page: Page, labels: string[]) {
...
export async function setFileInput(locator: Locator, filePath: string) {
...
export async function extractBasicFormFields(page: Page): Promise<DetectedFormField[]> {
  const fields = await page.locator("input, textarea, select").evaluateAll((elements) =>
    elements.map((element) => {
      ...
      const sameNamedInputs =
        typeAttr === "radio" || typeAttr === "checkbox"
          ? Array.from(document.querySelectorAll(`input[type="${typeAttr}"]`)).filter(
...
export async function fillTextLikeField(page: Page, field: DetectedFormField, value: string) {
...
export async function fillSelectLikeField(page: Page, field: DetectedFormField, value: string) {
  ...
  if (field.type === "radio" || field.type === "checkbox") {
    if (field.groupName) {
      ...
      for (const candidate of candidates) {
        ...
        if (
          normalizeFieldText(candidateValue) === normalizeFieldText(value) ||
...
export async function collectTextErrors(page: Page, selectors: string[]) {
  const messages: string[] = [];
  for (const selector of selectors) {
    const items = await page.locator(selector).evaluateAll((elements) =>
      elements
        .filter((element) => {
          const style = window.getComputedStyle(element);
          const rect = element.getBoundingClientRect();
