...
export async function waitForAnyVisible(page: Page, selectors: string[], timeout = 5000) {
...
export async function selectOptionWhenReady(locator: Locator, value: string) {
  const count = await locator.count().catch(() => 0);
  if (count === 0) {
    return false;
  }
...
export async function waitForInputValue(locator: Locator, expectedValue: string, timeout = 2000) {
  await locator
    .evaluate(
      (element, payload) =>
...
export async function waitForSelectValue(locator: Locator, timeout = 2000) {
  await locator
    .evaluate(
      (element, inputTimeout) =>
        new Promise<void>((resolve, reject) => {
          const startedAt = Date.now();
          const tick = () => {
...
export async function waitForAsyncValidation(page: Page) {
  await page
    .waitForFunction(
      () => {
...
export async function waitForPageStateChange(page: Page, previous: Pick<PageStateSnapshot, "url" | "title">, timeout = 5000) {
  await page
    .waitForFunction(
      (state) => document.location.href !== state.url || document.title !== (state.title ?? ""),
      previous,
      { timeout },
    )
    .catch(() => undefined);
