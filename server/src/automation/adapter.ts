...
import { capturePageState, type PageStateSnapshot } from "./shared/playwright-utils.js";
import { assertPageNotAccessBlocked } from "./shared/page-access-guard.js";

export interface AutomationContext {
  profile: Profile;
  resume?: StoredDocument;
  coverLetter?: StoredDocument;
...
export interface AdapterStepState {
  step: string;
  detail?: string;
}

export interface AdapterCollectedError {
  message: string;
  fieldLabel?: string;
}
...
export type ScreenshotHook = (name: string, page: Page) => Promise<void>;
export type StatusReporter = (update: AutomationStatusUpdate) => void;
...
export interface AtsAdapter {
  ...
  openJobPage(page: Page, url: string, context: AutomationContext): Promise<void>;
  extractJobMetadata(page: Page, url: string, context: AutomationContext): Promise<AdapterMetadata>;
  clickApply(page: Page, context: AutomationContext): Promise<void>;
  extractFormFields(page: Page, context: AutomationContext): Promise<DetectedFormField[]>;
  fillField(page: Page, field: DetectedFormField, value: string, context: AutomationContext): Promise<boolean>;
  uploadResume(page: Page, filePath: string, context: AutomationContext): Promise<boolean>;
  uploadCoverLetter(page: Page, filePath: string, context: AutomationContext): Promise<boolean>;
  answerScreeningQuestions(page: Page, context: AutomationContext): Promise<void>;
  reviewBeforeSubmit(page: Page, context: AutomationContext): Promise<void>;
  submitApplication(page: Page, context: AutomationContext): Promise<boolean>;
  getCurrentStep(page: Page, context: AutomationContext): Promise<AdapterStepState>;
  collectErrors(page: Page, context: AutomationContext): Promise<AdapterCollectedError[]>;

  extractMetadata?(page: Page, url: string): Promise<AdapterMetadata>;
  fillApplication?(page: Page, context: AutomationContext): Promise<AdapterRunResult>;
}

export async function runAdapterFlow(
  adapter: AtsAdapter,
  page: Page,
  jobUrl: string,
  context: AutomationContext,
): Promise<AdapterRunResult> {
  const statusUpdates: AutomationStatusUpdate[] = [];
  const fillMappedFields = async (fields: DetectedFormField[], passLabel: string) => {
    const requiredUnresolved = new Set<string>();
    ...
    const fillQueue = [...decisions].sort((left, right) => {
      const priority = (type: DetectedFormField["type"]) => {
    ...
    const criticalChoiceFields = decisions.filter((decision) =>
  ...
  const reportStatus = async (update: Omit<AutomationStatusUpdate, "url" | "pageState"> & { page?: Page }) => {
    const targetPage = update.page ?? page;
    const pageState = await capturePageState(targetPage);
    const fullUpdate: AutomationStatusUpdate = {
      stage: update.stage,
    ...
    context.logger?.info("Automation status update.", fullUpdate as unknown as Record<string, unknown>);
  };
  ...
  const unresolvedFields =
    finalPass.requiredUnresolvedFields.length > 0
      ? finalPass.requiredUnresolvedFields
      : initialPass.requiredUnresolvedFields;
  const confidencePenalty =
    Math.min(collectedErrors.length * 0.1, 0.3) + Math.min(unresolvedFields.length * 0.06, 0.24);
  const confidenceScore = Math.max(
    0.4,
    Math.min(summarizeMappingConfidence(mappingDecisions), 0.92) - confidencePenalty,
  );
  const reviewSummary = buildReviewSummary({
    ...
    unresolvedFields,
    validationErrors: collectedErrors.map((error) =>
      error.fieldLabel ? `${error.fieldLabel}: ${error.message}` : error.message,
    ),
    lastCompletedStep: step.step,
  });
  // #region agent log
  agentDebugLog({
    location: "adapter.ts:runAdapterFlow:preSubmit",
    message: "review gate before submitApplication",
    data: {
      hypothesisId: "H3",
      provider: adapter.provider,
      shouldAttemptSubmit: reviewSummary.shouldAttemptSubmit,
      confidenceScore,
      threshold: context.profile.autoSubmitConfidenceThreshold,
      unresolvedFields,
      validationErrorCount: collectedErrors.length,
      blockingReasons: reviewSummary.blockingReasons,
    },
  });
  // #endregion
  ...
  // #region agent log
  agentDebugLog({
    location: "adapter.ts:runAdapterFlow:postSubmit",
    message: "final automation status",
    data: {
      hypothesisId: "H4",
      provider: adapter.provider,
      submitted,
...
export function buildReviewSummary(params: {
  ...
  if (roundedConfidence < roundedThreshold) {
    blockingReasons.push(
      `Confidence ${roundedConfidence.toFixed(2)} is below the auto-submit threshold ${roundedThreshold.toFixed(2)}.`,
    );
  }

  const eligibleForAutoSubmit =
    params.unresolvedFields.length === 0 &&
    params.validationErrors.length === 0 &&
    roundedConfidence >= roundedThreshold;
  const shouldAttemptSubmit = eligibleForAutoSubmit;
  /** Gated skip after reaching the form: not a hard automation failure; user can finish manually or re-run. */
  const recommendedStatus: ApplicationStatus = shouldAttemptSubmit ? "Applied" : "Needs Review";
...
export async function safelyCount(locator: Locator) {
