...
test("returns preference and compliance answers from saved fields", () => {
    const service = new ScreeningAnswerService(profile);
    assert.equal(service.answerQuestion("Preferred programming language", "short").answer, "TypeScript");
    assert.equal(service.answerQuestion("Will you require sponsorship now or in the future?", "short").answer, "No");
    assert.equal(service.answerQuestion("Earliest start date", "short").answer, "2026-05-01");
    assert.equal(service.answerQuestion("Salary expectations", "short").answer, "$140,000");
});
