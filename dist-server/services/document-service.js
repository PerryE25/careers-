...
export class DocumentService {
    ...
    saveUploadedResume(file, profileId) {
        const ext = path.extname(file.originalname).toLowerCase();
        const mimeType = file.mimetype.toLowerCase();
        const supported = SUPPORTED_RESUME_EXTENSIONS.has(ext) ||
    ...
    selectResume(options) {
        const documents = this.store
            ...
            .filter((entry) => Boolean(entry.resolvedPath))
    ...
    resolveExistingDocumentPath(document) {
    ...
    toDescriptor(document, filePath = document.storagePath) {
        return {
            id: document.id,
            kind: document.kind,
    ...
    getOrCreateLocalDefaultResume(profileId) {
...
function getMimeTypeForResumeExtension(ext) {
    switch (ext) {
        case ".pdf":
...
function buildSafeCoverLetter(profile, job) {
