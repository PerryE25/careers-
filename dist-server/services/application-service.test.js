...
function createApplicationSeed(overrides = {}) {
    return {
...
function createService(store, overrides = {}) {
    ...
    return new ApplicationService(store, detector, registry, documents);
}
test("blocks duplicate automation when canonical ATS URL already exists", async () => {
    ...
    try {
        ...
        const job = store.upsertJob({ normalizedUrl: canonicalUrl }, {
            provider: "workday",
        ...
        assert.deepEqual(result.duplicateMatch?.reasons.map((reason) => reason.code), ["provider", "canonical-url"]);
    }
    finally {
...
test("allows unique automation when provider and normalized company/title match but external job ids differ", async () => {
    ...
    try {
        ...
        const job = store.upsertJob({ normalizedUrl: existingUrl }, {
            provider: "greenhouse",
...
test("normalized duplicate matching ignores company suffixes and punctuation", () => {
    ...
    try {
        ...
        assert.ok(match);
        assert.deepEqual(match?.reasons.map((reason) => reason.code), ["provider", "company-name", "job-title"]);
    }
    finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
    }
...
test("startBatch rejects when saved profile text is incomplete", () => {
    ...
    const service = createService(store);
    try {
        assert.throws(() => service.startBatch(["https://jobs.lever.co/acme/123"]), /saved master resume text/i);
    }
    finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
    }
...
test("startBatch rejects when saved autofill text is malformed", () => {
    ...
    try {
        assert.throws(() => service.startBatch(["https://jobs.lever.co/acme/123"]), /fix your saved profile source/i);
    }
    finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
    }
