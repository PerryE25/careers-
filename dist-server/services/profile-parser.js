...
function extractMailtoAddress(value) {
...
function collectSections(autofillText) {
...
function collectAutofillFields(sections) {
    const fields = {};
...
function parseAnswerLines(lines) {
    const answers = [];
...
function parseProjectsLines(lines) {
    return lines
        .map((line) => line.replace(/^[-*]\s*/, "").trim())
        .filter(Boolean)
        .map((line) => {
        ...
        const technologies = unique(splitList(summary)
...
function categorizeSkillLine(value, skills) {
...
function mergeResumeIntoCanonical(resumeText, canonical) {
    ...
    if (canonical.technicalSkills.languages.length === 0 &&
...
function validateCanonicalProfile(canonical) {
    const issues = [];
...
function fromSections(sections, autofillFields) {
    ...
    const demographicAnswers = Object.fromEntries(Object.entries(autofillFields)
    ...
    const relocationLocations = unique([
        ...
        ...(sections.relocation_preferences ?? [])
            ...
            .filter((pair) => Boolean(pair && pair.key.includes("location")))
            .flatMap((pair) => splitList(pair.value)),
    ]);
    const hasExplicitNoWorkExperience = asBoolean(autofillFields.no_work_experience) === true ||
...
function scorePromptMatch(query, candidate) {
...
export class StructuredProfileHelper {
    ...
    getBestAnswerForField(fieldLabel, fieldType, options = []) {
        ...
        if (label.includes("u_s_person") ||
            label.includes("us_person") ||
            label.includes("u s person") ||
            label.includes("us person")) {
            const usPerson = asBoolean(this.profile.autofillFields.u_s_citizen) ??
        ...
        if (/(onsite|on site|in person)/i.test(label)) {
            const openToOnsite = asBoolean(this.profile.autofillFields.open_to_onsite) ??
    ...
    getAnswerForPrompt(prompt, preferredLength) {
...
export function parseProfileText(resumeText, autofillText) {
