...
export class JsonStore {
    ...
    read() {
        ...
        return {
            ...
            applications: (parsed.applications ?? []).map((application) => ({
                ...application,
                status: application.status === "Needs Review" ? "Applied" : application.status,
                ...
                reviewSummary: application.reviewSummary
                    ? {
                        ...
                        recommendedStatus: application.reviewSummary.recommendedStatus === "Needs Review"
                            ? "Applied"
                            : application.reviewSummary.recommendedStatus,
                    }
                    : undefined,
    ...
    findJobByNormalizedUrl(normalizedUrl) {
    ...
    findDuplicateApplication(input, options = {}) {
        ...
        const matches = db.applications
            ...
            .filter((application) => application.status !== "Duplicate" && application.status !== "Failed")
            .map((application) => ({
            application,
            ...
            const hasDistinctExternalJobId = Boolean(normalizedExternalJobId &&
    ...
    listApplications() {
        ...
        return db.applications
            ...
            const run = application.lastRunId
                ...
                : undefined;
            const failureLogs = (application.failureLogIds ?? [])
