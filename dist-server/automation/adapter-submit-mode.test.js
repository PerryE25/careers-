...
function createProfile(submitMode, autoSubmitConfidenceThreshold) {
    return {
        id: "profile-1",
        resumeText: "",
        autofillText: "",
...
function createContext(submitMode, threshold) {
    return {
...
function makeField(label, required = true) {
    return {
        label,
        type: "text",
        required,
...
test("Valid runs submit successfully even when the saved mode was review", async () => {
    ...
    try {
        ...
        const result = await runAdapterFlow(adapter, page, "https://jobs.lever.co/acme/123", createContext("review", 0.75));
...
test("Low-confidence runs fail instead of entering a review state", async () => {
    const browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    let submitCount = 0;
    try {
        ...
        const result = await runAdapterFlow(adapter, page, "https://jobs.lever.co/acme/123", createContext("auto", 0.9));
        assert.equal(submitCount, 0);
        assert.equal(result.submitAttempted, false);
        assert.equal(result.submitCompleted, false);
        assert.equal(result.status, "Failed");
        assert.ok(result.reviewSummary?.blockingReasons.some((reason) => /below the auto-submit threshold/i.test(reason)));
    }
    finally {
        await browser.close();
    }
});
test("Required field blockers fail instead of entering a review state", async () => {
    const browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    let submitCount = 0;
    try {
        ...
        const result = await runAdapterFlow(adapter, page, "https://jobs.lever.co/acme/123", createContext("auto", 0.4));
        assert.equal(submitCount, 0);
        assert.equal(result.submitAttempted, false);
        assert.equal(result.status, "Failed");
