...
const DEMOGRAPHIC_FIELD_PATTERN = /\b(demographic|eeo|gender|sex|race|ethnicity|hispanic|latino|veteran|disability|self identify|self-identify|sexual orientation|lgbtq|pronoun)\b/i;
...
export function normalizeFieldLabel(input) {
    return input
        .trim()
        .toLowerCase()
        .replace(/\([^)]*\)/g, " ")
...
function matchCanonicalKey(normalizedLabel) {
...
function normalizeOptionMatch(value, options) {
...
function isDemographicField(field, normalizedLabel) {
...
function isDemographicOptOutOption(option) {
    const normalizedOption = normalizeFieldLabel(option);
    return DEMOGRAPHIC_OPT_OUT_PATTERNS.some((pattern) => pattern.test(normalizedOption));
}
function findSavedDemographicAnswer(profile, normalizedLabel, options) {
...
function findDemographicAnswer(profile, field, normalizedLabel) {
...
function computeConfidence(field, normalizedLabel, canonicalKey, answer) {
    if (!answer) {
        return 0.1;
    }
    let score = 0.45;
...
function getAnswerByCanonicalKey(profile, canonicalKey, field) {
    if (!canonicalKey) {
        return undefined;
    }
    const canonical = profile.canonicalProfile;
    const optionText = (value) => value
...
export function mapFieldToProfile(profile, field) {
    ...
    if (profile.canonicalProfile.hasExplicitNoWorkExperience &&
    ...
    if (/(why do you want|tell us about yourself|describe|additional information|motivation)/i.test(normalizedLabel) &&
        !answer) {
...
function findPromptAnswer(profile, prompt, preferredLength) {
    ...
    const sameLength = profile.canonicalProfile.prewrittenAnswers.find((candidate) => candidate.length === preferredLength &&
        normalizedPrompt.split(" ").some((term) => term && normalizeFieldLabel(candidate.prompt).includes(term)));
    return sameLength?.answer;
}
export function summarizeMappingConfidence(decisions) {
    ...
    const averageConfidence = resolved.reduce((sum, decision) => sum + decision.confidence, 0) / resolved.length;
    ...
    const weightedScore = averageConfidence * 0.8 +
