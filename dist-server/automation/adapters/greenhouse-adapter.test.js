...
import { runAdapterFlow } from "../adapter.js";
...
function createTempDocuments(prefix) {
...
test("Greenhouse adapter completes a hosted happy path fixture and submits successfully", async () => {
    ...
    try {
        ...
        assert.equal(await page.locator("#resume").evaluate((el) => el.files?.length ?? 0), 1);
    }
    finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
        await browser.close();
...
test("Greenhouse adapter treats custom gh_jid pages with inline forms as ready-to-fill applications", async () => {
    ...
    try {
        ...
        const metadata = await adapter.extractJobMetadata(page, "https://bolt.eu/en/careers/positions/8488707002/?gh_jid=8488707002", context);
...
test("Greenhouse adapter auto-submits inline custom forms with optional demographic sections", async () => {
    ...
    try {
        ...
        const result = await runAdapterFlow(adapter, page, "https://bolt.eu/en/careers/positions/8488707002/?gh_jid=8488707002", context);
        assert.equal(result.status, "Applied");
        assert.equal(result.submitAttempted, true);
        assert.equal(result.submitCompleted, true);
        assert.deepEqual(result.unresolvedFields, []);
