...
test("Workday adapter extracts metadata and detects current step", async () => {
    ...
    try {
        ...
        const metadata = await adapter.extractJobMetadata(page, "https://acme.wd5.myworkdayjobs.com/en-US/External/job/Remote/Senior-Analytics-Engineer_JR-9001", context);
...
test("Workday adapter progresses through steps and submits successfully", async () => {
    ...
    try {
        ...
        assert.equal(await page.locator("#coverUpload").evaluate((el) => el.files?.length ?? 0), 1);
    }
    finally {
...
test("Workday adapter fails safely on unresolved required fields", async () => {
    const browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    const adapter = new WorkdayAdapter();
    try {
